...
class DiMP(BaseTracker):
    ...
    def _inject_curvature_optimizer(self):
        """Inject CurvatureAwareDiMP into the classifier in-place.

        Replaces the baseline DiMPSteepestDescentGN / DiMPL2SteepestDescentGN
        optimizer inside self.net.classifier.filter_optimizer with a
        CurvatureAwareDiMP instance that carries over all trained parameters
        (filter_reg, step_length, label_map_predictor, etc.) from the original
        optimizer while adding curvature-aware regularization on top.

        Anchor state (H_diag_ema, weights_anchor) is kept as non-learnable
        registered buffers so that it is sequence-local and does not pollute
        the pre-trained optimizer weights.
        """
        import ltr.models.target_classifier.optimizer as clf_optimizer

        orig = self.params.net.classifier.filter_optimizer

        # Mirror the original optimizer's __init__ signature so all pre-trained
        # parameters (filter_reg, log_step_length, label_map_predictor, etc.)
        # are copied across without re-initialisation.
        new_opt = clf_optimizer.CurvatureAwareDiMP(
            num_iter=orig.num_iter,
            feat_stride=orig.feat_stride,
            init_step_length=torch.exp(orig.log_step_length).item(),
            init_filter_reg=orig.filter_reg.item(),
            init_gauss_sigma=orig.gauss_sigma if hasattr(orig, 'gauss_sigma') else 1.0,
            num_dist_bins=orig.distance_map.num_bins,
            bin_displacement=orig.distance_map.bin_displacement,
            mask_init_factor=4.0,
            score_act='relu',
            act_param=None,
            min_filter_reg=orig.min_filter_reg,
            mask_act='sigmoid',
            detach_length=orig.detach_length,
            alpha_eps=orig.alpha_eps,
            use_curvature_reg=self.params.get('use_curvature_reg', False),
        ...
        # [STATE-AWARE] Inject per-instance thresholds into the optimizer so
        # _get_curvature_state() can read them without polluting the class __init__.
        new_opt._curv_low_score_th  = self.params.get('curv_low_score_th',  0.35)
        new_opt._curv_hard_weight   = self.params.get('curv_hard_weight',   10.0)
        new_opt._curv_soft_weight   = self.params.get('curv_soft_weight',   1.0)
        ...
        new_opt.label_map_predictor.load_state_dict(orig.label_map_predictor.state_dict())
        new_opt.target_mask_predictor.load_state_dict(orig.target_mask_predictor.state_dict())
        new_opt.spatial_weight_predictor.load_state_dict(orig.spatial_weight_predictor.state_dict())
        new_opt.log_step_length.data = orig.log_step_length.data.clone()
        new_opt.filter_reg.data = orig.filter_reg.data.clone()
        if hasattr(orig, 'score_activation'):
            new_opt.score_activation = orig.score_activation
            new_opt.score_activation_deriv = orig.score_activation_deriv

        # Move to the same device / dtype as the original
        new_opt = new_opt.to(orig.label_map_predictor.weight.device)
        new_opt.train(orig.training)

        self.params.net.classifier.filter_optimizer = new_opt
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # Inject curvature-aware optimizer if enabled.
        # We do this here (after net is loaded from checkpoint) so that:
        #  1. We benefit from all pre-trained DiMP weights unchanged
        #  2. The anchor buffers (H_diag_ema, weights_anchor) start fresh per sequence
        ucr = self.params.get('use_curvature_reg', False)
        if ucr:
            self._inject_curvature_optimizer()

        # The DiMP network
        self.net = self.params.net

        # [CURVATURE-AWARE] Reset anchor state for each new tracking sequence.
        # This must happen AFTER self.net is assigned so we reset the correct optimizer instance.
        if ucr:
            self.net.classifier.filter_optimizer.reset_anchor()

        # Diagnostic logger switch (kept off by default to avoid runtime overhead).
        self.diag_debug_mode = self.params.get('diag_debug_mode', False)
        self._last_optimizer_diag_state = None
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      ...
                                                                      self.img_sample_sz)
        # [NEW] Store for H_diag alignment profiler
        self._last_sample_coords = sample_coords
        self._last_im_patches = im_patches
        ...
        if self.diag_debug_mode:
            ...
            if diag_state.get('max_score', None) is None:
                diag_state['max_score'] = float(max_score)
            if diag_state.get('anchor_updated', None) is None:
                diag_state['anchor_updated'] = False
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        # [CURVATURE-AWARE] Anchor must NOT be initialised from augmented samples —
        # those are geometrically distorted (flips, rotations, dropout) and would
        # corrupt the slow anchor.  Anchor updates are deferred to the first clean
        # normal update_classifier() call.
        # Only pass update_anchor when curvature-aware optimizer is injected.
        get_filter_kwargs = {'num_iter': num_iter, 'compute_losses': plot_loss}
        if self.params.get('use_curvature_reg', False):
            get_filter_kwargs['update_anchor'] = False
        with torch.no_grad():
            self.target_filter, _, losses = self.net.classifier.get_filter(x, target_boxes, **get_filter_kwargs)
    ...
    def update_classifier(self, train_x, target_box, learning_rate=None, scores=None):
        ...
        if num_iter > 0:
            ...
            # [CURVATURE-AWARE] update_anchor=True only during clean normal updates,
            # not during hard-negative mining where samples are noisy.
            update_anchor = (not hard_negative_flag) and self.params.get('use_curvature_reg', False)
            opt_kwargs = dict(
                num_iter=num_iter, feat=samples, bb=target_boxes,
                sample_weight=sample_weights, compute_losses=plot_loss,
            )
            opt_forward_params = inspect.signature(self.net.classifier.filter_optimizer.forward).parameters
            if 'scores' in opt_forward_params:
                opt_kwargs['scores'] = scores   # [STATE-AWARE] frame-level max_score for shield decision
            if self.params.get('use_curvature_reg', False) and 'update_anchor' in opt_forward_params:
                opt_kwargs['update_anchor'] = update_anchor
            with torch.no_grad():
                self.target_filter, _, losses = self.net.classifier.filter_optimizer(
                    self.target_filter, **opt_kwargs)
            self._last_optimizer_diag_state = getattr(self.net.classifier.filter_optimizer, '_last_diag_state', None)
