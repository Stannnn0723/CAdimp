...
def load_network(net_path, **kwargs):
    ...
    if os.path.isabs(net_path):
        path_full = net_path
        print(f"[pytracking] Loading checkpoint from absolute path: {path_full}  (file: {os.path.basename(path_full)})")
        ...
        for p in env_settings().network_path:
            ...
            try:
                print(f"[pytracking] Loading checkpoint from list path: {path_full}  (file: {os.path.basename(path_full)})")
                net, _ = ltr_loading.load_network(path_full, **kwargs)
                break
        ...
        print(f"[pytracking] Loading checkpoint from: {path_full}  (file: {os.path.basename(path_full)})")
        net, _ = ltr_loading.load_network(path_full, **kwargs)
