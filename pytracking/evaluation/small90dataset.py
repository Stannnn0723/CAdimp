...
class Small90Dataset(BaseDataset):

    def __init__(self):
        super().__init__()
        self.base_path = self.env_settings.small90_path
    ...
    def _construct_sequence(self, sequence_info):
        ...
        imgs = [f for f in os.listdir(full_img_path) if f.endswith(('.jpg', '.png', '.jpeg', '.bmp', '.JPG', '.PNG', '.JPEG', '.BMP'))]
        imgs = sorted(imgs)
        ...
        return Sequence(
            sequence_info['name'], frames, 'small90',
    ...
    def _get_sequence_info_list(self):
        ...
        for seq_name in sequence_dirs:
            ...
            try:
                ...
                num_frames = len([f for f in files if f.endswith(('.jpg', '.png', '.jpeg', '.bmp', '.JPG', '.PNG', '.JPEG', '.BMP'))])
            except Exception as e:
