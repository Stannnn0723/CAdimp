...
class UAVDataset(BaseDataset):
    ...
    """
    def __init__(self, attribute=None, split='all'):
        super().__init__()
        ...
        if split == 'uav20l':
            # Long sequences: frame count > 900 (≈1000 frames at 30fps)
            self.sequence_info_list = [
                s for s in self.sequence_info_list
                if (s['endFrame'] - s['startFrame']) > 900
            ]
        elif split == 'uav10fps':
            # 10fps subset: same sequences, frames are sub-sampled at 1/3 rate
            pass  # handled in _construct_sequence via self.split check
    ...
    def _construct_sequence(self, sequence_info):
        ...
        # UAV10fps: sub-sample to ~10fps by taking every 3rd frame
        if getattr(self, 'split', 'all') == 'uav10fps':
            frames = frames[::3]
            ground_truth_rect = ground_truth_rect[::3]
