...
def trackerlist(name: str, parameter_name: str, run_ids = None, display_name: str = None,
    ...
    args:
        ...
        dataset_name: Name of dataset (used to organize results into subdirectories).
    """
    if run_ids is None or isinstance(run_ids, int):
        run_ids = [run_ids]
    return [Tracker(name, parameter_name, run_id, display_name, dataset_name) for run_id in run_ids]
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, run_id: int = None, display_name: str = None,
        ...
        env = env_settings()
        base = '{}/{}/{}'.format(env.results_path, self.name, self.parameter_name)
        seg_base = '{}/{}/{}'.format(env.segmentation_path, self.name, self.parameter_name)
        if self.dataset_name:
            base = '{}/{}'.format(base, self.dataset_name)
            seg_base = '{}/{}'.format(seg_base, self.dataset_name)
        if self.run_id is None:
            self.results_dir = base
            self.segmentation_dir = seg_base
        else:
            self.results_dir = '{}_{:03d}'.format(base, self.run_id)
            self.segmentation_dir = '{}_{:03d}'.format(seg_base, self.run_id)
    ...
    def run_video_generic(self, debug=None, visdom_info=None, videofilepath=None, optional_box=None, save_results=False):
        ...
        if save_results:
            ...
            for obj_id, bbox in output_boxes.items():
                tracked_bb = np.array(bbox).astype(float)
                bbox_file = '{}_{}.txt'.format(base_results_path, obj_id)
                np.savetxt(bbox_file, tracked_bb, delimiter='\t', fmt='%.4f')
